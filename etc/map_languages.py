...
# manually overwrite geocoordinates
coordinates = {}
with open(Path(__file__).parent / "coordinates.tsv") as f:
    reader = csv.DictReader(f, delimiter="\t")
    for row in reader:
        coordinates[row["Name"]] = (row["Latitude"], row["Longitude"])
    ...
    for row in reader:
        ...
        try:
            ...
            if name in coordinates:
                row["Latitude"], row["Longitude"] = coordinates[name]
            else:
